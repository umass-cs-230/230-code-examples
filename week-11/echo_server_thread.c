...
void *thread(void *vargp) {
  ...
  // Detach from main thread - sets up auto reaping.
  pthread_detach(pthread_self());
  // Free the memory associated with the file descriptor argument.
  free(vargp);
  // Service the request with echo.
  echo(connfd);
  // Close the connection once we return from echo.
  close(connfd);
...
int open_listenfd(int port) {
  ...
  // Server address information (protocol family, port, etc.).
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    // };
    int *connfd = malloc(sizeof(int));  // the connection file descriptor
    *connfd = accept(listenfd, (struct sockaddr *)(&clientaddr), &clientlen);
    ...
    // Spawn the thread to service the request.
    pthread_t tid;
